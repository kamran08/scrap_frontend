                        ...
                        <div class="_sign_r8_main">
                            ...
                            <div class="_sign_r8_sngl_all"> 
                                ...
                                <div class="_sign_r8_sngl" id="divOuter">
                                    <div id="divInner">
                                        <p>Enter your code</p>
                                        <input id="partitioned" @keyup="error =''" v-model="token" type="text" maxlength="6" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  onKeyPress="if(this.value.length==6) return false;" /> 
                                ...
                                <div class="_sign_r8_btn" >
                                    <Button type="button" @click="verify()" :loading="isLoading" :disabled="isLoading">Verify email</Button>
                                </div>
...
export default {
  ...
  methods:{
    async verify() {
        ...
        // if(flag==0) return
        this.isLoading = true;
        const res = await this.callApi('post','auth/verify_pass_reset_code', {verificationCode:this.token})
        if(res.status==204){
